...
async function refreshAll() {
    try {
        console.log("refreshAll: Загружаем данные с сервера...");

        const [a, g, b, c] = await Promise.all([
            ...
            fetchJson("/api/comments").catch(e => { console.warn("comments failed", e); return []; }),
        ]);
        ...
        console.log("refreshAll: authors", authors);
        console.log("refreshAll: genres", genres);
        console.log("refreshAll: books", books);
        console.log("refreshAll: comments", comments);
        ...
        console.log("refreshAll: Данные обновлены и UI перерисован.");
    } catch (e) {
...
function renderComments() {
    ...
    books.forEach(b => {
        ...
        list.forEach((text, idx) => {
            ...
            editBtn.textContent = "Edit";
            editBtn.onclick = () => {
                console.log("EditBtn click:", {commentId, bookId: b.id, text});
                editComment(commentId, b.id, text);
            };
            const delBtn = document.createElement("button");
            delBtn.textContent = "Delete";
...
async function editComment(commentId, bookId, oldText) {
    console.log("editComment invoked with:", {commentId, bookId, oldText});
    if (!commentId) {
        alert("No comment ID");
        return;
    }
    const newText = prompt("Edit comment text:", oldText);
    console.log("Prompt result:", newText);
    if (newText !== null && newText.trim()) {
        try {
            ...
            console.log("Sending PUT for comment edit:", body);
